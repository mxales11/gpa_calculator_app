...
module TakenCoursesHelper
	...
	def taken_courses_list_item(taken_course)
		...
		out+=link_to('Edit', edit_student_taken_course_path(@student, taken_course))
		out+="  "
		out+=link_to('Destroy', student_taken_course_path(@student, taken_course), :class => "small-link", confirm: 'Are you sure?', method: :delete, :remote => true)
		out+="</li></div>"
		out.html_safe
	end
